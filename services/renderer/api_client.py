"""Renderer-side API client for authenticated render job operations."""
...
class RenderApiClient:
    ...
    def claim_job(self, job_id: int, *, lease_seconds: int) -> dict[str, Any]:
        resp = self.session.post(
            f"{self.base_url}/render-jobs/{job_id}/claim",
    ...
    def heartbeat(self, job_id: int) -> dict[str, Any]:
        resp = self.session.post(
            f"{self.base_url}/render-jobs/{job_id}/heartbeat",
            timeout=30,
            headers=auth_headers(),
        )
        return self._raise_or_json(resp)
    ...
    def set_status(self, job_id: int, payload: dict[str, Any]) -> dict[str, Any]:
        resp = self.session.post(
            f"{self.base_url}/render-jobs/{job_id}/status",
            json=payload,
            timeout=30,
            headers=auth_headers(),
        )
    ...
    def get_context(self, job_id: int) -> dict[str, Any]:
        resp = self.session.get(
            f"{self.base_url}/render-jobs/{job_id}/context",
            timeout=30,
            headers=auth_headers(),
        )
        resp.raise_for_status()
...
__all__ = ["RenderApiClient", "auth_headers"]
